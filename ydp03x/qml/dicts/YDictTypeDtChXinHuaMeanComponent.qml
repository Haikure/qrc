...
Repeater{
    ...
    property var contentWidth: 538 + 18 +26
    property var meanList: []
    property var meanListXmlData: {

        return spliteXmlNodeToGetText(dictSelectParaphrasesXml,"义项")
    }
    model:  meanList
    ...
    Component {
        ...
        Item {
            width: contentWidth
            height:id_meaning_column.height + 20
            Column {
                ...
                property bool haveMeanText: {
                    ...
                    try {
                        if( JSON.stringify(id_meaning_column.modelModelData.用法).length)
                            temp = true
                    } catch(e) {}
                    ...
                    //                    try{
                    //                        if(id_meaning_column.splitWordMeanList.length) {
                    //                            temp = true
                    //                        }} catch(e) {}
                ...
                Item{
                    ...
                    Column {
                        ...
                        property var curDataJson: {
                            ...
                            try {
                                if(typeof id_meaning_column.modelModelData.释义 !== "undefined") {
                                    ...
                                    try {
                                        //if(index === 0)
                                        //var temp3 = spliteXmlNodeToGetText(dictSelectParaphrasesXml,"义项")
                                        //console.warn("meanListXmlData,data:emp2:"+JSON.stringify(temp3)+",curIndex："+curIndex)
                                        //console.warn("meanListXmlData,data:temp2[curIndex].subStr:"+JSON.stringify(temp3[curIndex].subStr))
                                        let meanTemp = spliteXmlNodeToGetText(meanListXmlData[curIndex].subStr,'释义')
                                        //console.warn("meanTemp[0]:"+JSON.stringify(meanTemp[0]))
                                        var temp2 = ""
                                        if(typeof meanTemp[0].subStr !== "undefined")
                                            temp2 = meanTemp[0].subStr.replace(/<.*?>/g,'') .trim()
                                        //console.warn("remove,meanTemp[0]:"+JSON.stringify(temp2))
                                        //                                    var temp2 = getMeanItemSearchXmlData('字目/释文', dictSelectParaphrasesXml)
                                        //                                     temp2 = getMeanSearchXmlData(temp2,"义项")
                                        //                                     temp2 = getMeanDataText(temp2[curIndex][1],"释义")
                                        if(temp2.length) {
                                            m["释义data"] = temp2
                                            temp["释义"] = m
                                        }
                            ...
                            try {
                                if(typeof id_meaning_column.modelModelData.语体 !== "undefined") {
                                    if ( typeof  temp["语体"] === "undefined")
                                        temp["语体"] = {}
                                    tempTextData = id_dict_type_ch_ancientword.getKeyOrText(id_meaning_column.modelModelData.语体)
                                    if(tempTextData !== null)
                                        temp["语体"]["语体data"] = tempTextData
                                }
                            } catch(e) {}

                            try {
                                try {
                                    let temp2 = getMeanItemSearchXmlData('字目/释文', dictSelectParaphrasesXml)
                                    temp2 = getMeanSearchXmlData(temp2,"义项")
                                    temp2 = getMeanDataText(temp2[curIndex][1],"注")
                                    if(temp2.length) {
                                        temp["释义"]["注"]["meanMention"] = temp2
                                    }
                                } catch(e) {
                            ...
                            try {
                                if(typeof id_meaning_column.modelModelData.释义.SUB !== "undefined") {
                                    if ( typeof  temp["释义"] === "undefined")
                                        temp["释义"] = {}
                                    if ( typeof  temp["释义"]["SUB"] === "undefined")
                                        temp["释义"]["SUB"]  = {}
                                    tempTextData = id_dict_type_ch_ancientword.getAllValueString(id_meaning_column.modelModelData.释义.SUB)
                                    if(tempTextData !== null)
                                        temp["释义"]["SUB"]["SUBData"] = tempTextData
                                }
                            } catch(e) {}

                            try {
                                if(typeof id_meaning_column.modelModelData.释义.SUP !== "undefined") {
                                    if ( typeof  temp["释义"] === "undefined")
                                        temp["释义"] = {}
                                    if ( typeof  temp["释义"]["SUP"] === "undefined")
                                        temp["释义"]["SUP"]  = {}
                                    tempTextData = id_dict_type_ch_ancientword.getAllValueString(id_meaning_column.modelModelData.释义.SUP)
                                    if(tempTextData !== null)
                                        temp["释义"]["SUP"]["SUPData"] = tempTextData
                                }
                            } catch(e) {}
                            ...
                            try {
                                try {
                                    ...
                                    let meanTemp = spliteXmlNodeToGetText(meanListXmlData[curIndex].subStr,'例证')
                                    var temp2 = ""
                                    if(typeof meanTemp[0].subStr !== "undefined")
                                        temp2 = meanTemp[0].subStr.replace(/<.*?>/g,'') .trim()
                                    //                                let temp2 = getMeanItemSearchXmlData('字目/释文', dictSelectParaphrasesXml)
                                    //                                 temp2 = getMeanSearchXmlData(temp2,"义项")
                                    //                                 temp2 = getMeanDataText(temp2[curIndex][1],"例证")
                                    if(temp2.length) {
                                        temp["例证"] = {"例证data":temp2}
                                    }
                                } catch(e) {
                                    if(typeof id_meaning_column.modelModelData.例证 !== "undefined") {
                                        ...
                                        if(tempTextData !== null)
                                            temp["例证"] = {"例证data":tempTextData.replace('null','')}
                                    }
                                }
                            } catch(e) {}
                        ...
                        Flow {
                            ...
                            anchors.left: parent.left
                            width: id_mean_column.widthColumn
                        ...
                        YSpacingForColumn {
                            height: 8
                            visible: (id_word_mean_flow.firstMean.length || id_word_mean_flow.nextMean.length) &&  id_mean_use_method_value.visible
                        }

                        //用法
                        YText {
                            id: id_mean_use_method_value
                            width: visible ? (Math.min(id_meanings_repeater.contentWidth, paintedWidth))  : 0
                            font.family: fontManager.fontFamilyXinHuaXiHei
                            verticalAlignment: Text.AlignTop
                            font.pixelSize: 28
                            ...
                            text: {
                                let textTemp = ""
                                try {
                                    let tempTextData = id_dict_type_ch_ancientword.getKeyOrText(id_meaning_column.modelModelData.用法)
                                    if(tempTextData !== null) {
                                        textTemp = tempTextData
                                    }
                                } catch (e) {}
                        ...
                        Row{
                            id: id_extended_row
                            property bool visibleSet: {
                               return  typeof id_mean_column.curDataJson.引申 !== "undefined" && id_mean_column.curDataJson.引申.引申data.length
                            }
                            anchors.left: parent.left
                            anchors.right: parent.right
                ...
                YSpacingForColumn {
                    height: 8
                    visible: id_split_word_rectangle_repeater.count
                }

                //分义项
                YDictTypeDtChXinHuaSpliteMeanComponent {
