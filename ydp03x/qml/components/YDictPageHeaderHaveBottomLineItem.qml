...
YMouseArea {
    ...
    property var letterSpace:{
        if(1 === qmlGlobal.scanOldType && charType === YEnum.CT_KO){
            if(rectNextIsNotNewWord) return 0
            return 12
        }
        return 0 === qmlGlobal.scanOldType ? 20: (nextIsNotPunc && id_word_bg.clickable && charType === YEnum.CT_ENG ? 12 : 12)
    }
    ...
    property bool pointSearchSelectIndex: {

        return (0 === qmlGlobal.scanOldType && resultManager.mainQueryBreakList.length > 0 &&  !id_dict_listview.pointScanIsClick &&
                                           ((isAutoBreakWords && pointSelectIndex.indexOf(newBreakIndexToRawIndex[index]) !== -1) ||
                                           resultManager.currentQuery === orgBreakList[newBreakIndexToRawIndex[index]].content))
    }
    property bool pointSearchNotSelectIndex: (0 === qmlGlobal.scanOldType && resultManager.mainQueryBreakList.length > 0 &&  !id_dict_listview.pointScanIsClick &&
                                           !((isAutoBreakWords && pointSelectIndex.indexOf(newBreakIndexToRawIndex[index]) !== -1) ||
                                           resultManager.currentQuery === orgBreakList[newBreakIndexToRawIndex[index]].content))
    ...
    Item {
        ...
        height: id_word_bg.height + id_line.height /*+id_line_space.height*/
        clip: true
...
//                    font.bold: font.family === fontManager.fontFamilyXinHuaXiHei
                    font.letterSpacing: {
                        switch (charType) {
                        case YEnum.CT_CJK:
                            return 0
...
//                id: id_line_space
//                height: 2
//                visible: !isDetailShow && id_line.visible
//            }

            Rectangle {
                id: id_line
                width: id_word_bg.implicitRectWidth + (!id_word_bg.nextIsNotNewWord ? 0 : letterSpace)
                height: 2
                visible: {

                    let qsWord = resultManager.currentQuery

                    //如果不能分词，不显示
                    if (YEnum.WGT_Sentence === resultManager.currentQueryType && id_breakList_repeter.count === 1) {
                         var isSpliteWord =  breakWords(qsWord)
                         if (isSpliteWord === null)
                             return false
                    }

                    return id_word_bg.clickable &&  !isDetailShow && ((YEnum.WGT_Ch !== resultManager.currentQueryType && YEnum.WGT_En !== resultManager.currentQueryType) ||
                           pointSearchNotSelectIndex)
                }
                color: id_word.color === YColors.white ? YColors.white : "#878A99"
                opacity: 0.5
            }
        }
